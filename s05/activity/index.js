...
/*
    1. Create a function called "provideAccess" which is able to receive 2 parameters called username and role.
        ...
        - Add a else" statement which will contain another "if, else if and else" statement. 
            Add a condition to check the user's role adding 3 conditions for "admin", "manager" & "agent" and an "else" statement:
...
function provideAccess(username, role) {
    if(username == undefined || username == "" || role == undefined || role == "") {
        return "Username or Role is Empty";
    }
    else {
        if(role === "admin") {
...
message = provideAccess("Michael Lester Aguhob","admin");
console.log(message);
...
function earthquakeAlert(intensity1, intensity2, intensity3, intensity4) {
    let averageIntensity;
    averageIntensity = (intensity1 + intensity2 + intensity3 + intensity4) / 4;
    ...
    if(averageIntensity <= 3.9) {
        ...
        return "The magnitude level is: "+ averageIntensity +". Great earthquake detected. Please be aware of falling debris and imminent danger.";
    }
}

message = earthquakeAlert(1.7, 2.3, 7.0, 2.5);
console.log(message);